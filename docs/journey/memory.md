...
- **Phase 5 semantic-tree gutter alignment bug fixed and reviewed.** User screenshot showed active/pinned cyan gutter markers floating on empty raw-source lines below the selected semantic row. Frontend Engineer diagnosed the root cause: the gutter still used raw source line count/ranges after the left pane became a compressed source-backed semantic tree. Multi-line semantic nodes therefore marked detached raw source lines. Fix accepted by Frontend Expert/Code Reviewer and E2E/UX QA: gutter rows now mirror visible semantic rows/`+N more` rows, marker state is keyed by semantic node ID, and expanded container previews show only the original opening delimiter to avoid duplicated child content. Coordinator validation passed: `npm run typecheck`, `npm run test` (8 files, 158 tests), `npm run build`, `npm run build --workspace=@mvviewer/web`, `npm run e2e` (49 passed), `git diff --check`, and required static greps with only expected test matches. Non-blocking follow-ups: consider mobile inline-card/gutter spacer coverage and explicit `tree-row-more` row metrics if future visual QA finds drift.
