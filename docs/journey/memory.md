...
- **App shell discoverability metadata added.** The app remains the landing page, but `apps/web/index.html` now has a stronger title, meta description, canonical URL, Open Graph/Twitter metadata, SoftwareApplication JSON-LD, and crawlable initial product copy. Added `apps/web/public/llms.txt` so the static build publishes LLM/agent-oriented project context at `/manifest-lens/llms.txt`. Added a Playwright-generated 1200×630 social preview image at `apps/web/public/social-card.png`, wired it into Open Graph/Twitter/JSON-LD metadata, and added `npm run generate:social-card` for regeneration. Sitemap remains intentionally skipped because the deployed product is a single-page app.
...
- **Manifest knowledge coverage audit and first expansion slice completed.** Added `docs/knowledge/manifest-knowledge-sources.md` and `docs/knowledge/manifest-coverage.md` as the durable source inventory and coverage matrix for keeping Manifest Lens explanations current, grounded in reviewed Chrome Extension and MDN WebExtensions manifest, permissions, and match-pattern docs as of 2026-07-19. Refactored `packages/knowledge` so registry entry data lives under `src/entries/`, shared types/builders are separate, and `index.ts` remains the resolver/facade. Added checked-in coverage catalogs and tests under `src/coverage-catalog/`. Coverage increased from 36 to 125 path entries and from 4 to 85 permission entries, covering the reviewed Chrome and MDN top-level manifest keys, common and long-tail nested paths, Chrome/MDN permission strings, optional permissions/host permissions, Firefox Gecko settings, ChromeOS/browser-specific keys, and wildcard dynamic paths such as command names and icon sizes. Semantic recognition was expanded so covered top-level keys render as known fields, and optional permission/host-permission array items classify like install-time permission arrays. Per user request, no additional exhaustive test cases were added for the long-tail expansion yet; future hardening should extend coverage catalogs/tests and add value-level enum explanations once semantic value support is ready.

- **GitHub Pages deployment workflow added and verified.** GitHub Pages deploys from Actions, not from a checked-in root/docs build folder. Workflow `.github/workflows/pages.yml` runs only on push to `main`, installs with `npm ci`, runs `npm run typecheck`, `npm run test`, `npm run build`, builds `@manifest-lens/web` with `--base=/manifest-lens/`, uploads `apps/web/dist`, and deploys via official Pages actions. Local validation passed for the exact CI web build command: `npm run build --workspace=@manifest-lens/web -- --base=/manifest-lens/`. First Actions run failed because the new repo did not yet have Pages enabled; Pages was enabled once via GitHub API with `build_type=workflow`, the workflow rerun passed, and `https://iamaamir.github.io/manifest-lens/` returned HTTP 200. Follow-up CI maintenance updated official action versions to `actions/checkout@v5`, `actions/setup-node@v5`, `actions/configure-pages@v6`, `actions/upload-pages-artifact@v5`, and `actions/deploy-pages@v5`; the Pages workflow passed after the update and no Node 20 deprecation annotations appeared in the watched run output.
