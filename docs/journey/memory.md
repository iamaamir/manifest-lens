...
- **Manifest knowledge coverage audit started.** Added `docs/knowledge/manifest-knowledge-sources.md` and `docs/knowledge/manifest-coverage.md` as the durable source inventory and coverage matrix for keeping Manifest Lens explanations current. The audit is grounded in reviewed Chrome Extension and MDN WebExtensions manifest, permissions, and match-pattern docs as of 2026-07-19. Current registry coverage is explicitly recorded: 36 path entries and 4 permission entries, with many top-level keys, nested paths, permission values, and enum-like values marked missing/deferred/verify-source. Next recommended implementation slice is to add checked-in coverage catalogs and tests before expanding `packages/knowledge`.
