...
- **GitHub Pages deployment workflow added.** GitHub Pages will deploy from Actions, not from a checked-in root/docs build folder. Workflow `.github/workflows/pages.yml` runs on push to `main` plus manual dispatch, installs with `npm ci`, runs `npm run typecheck`, `npm run test`, `npm run build`, builds `@manifest-lens/web` with `--base=/manifest-lens/`, uploads `apps/web/dist`, and deploys via official Pages actions. Local validation passed for the exact CI web build command: `npm run build --workspace=@manifest-lens/web -- --base=/manifest-lens/`.
