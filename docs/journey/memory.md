...
- **Manifest knowledge coverage audit and first expansion slice completed.** Added `docs/knowledge/manifest-knowledge-sources.md` and `docs/knowledge/manifest-coverage.md` as the durable source inventory and coverage matrix for keeping Manifest Lens explanations current, grounded in reviewed Chrome Extension and MDN WebExtensions manifest, permissions, and match-pattern docs as of 2026-07-19. Refactored `packages/knowledge` so registry entry data lives under `src/entries/`, shared types/builders are separate, and `index.ts` remains the resolver/facade. Added checked-in coverage catalogs and tests under `src/coverage-catalog/`. Coverage increased from 36 to 69 path entries and from 4 to 20 permission entries, prioritizing common top-level keys, optional permissions/host permissions, Firefox Gecko settings, action/side-panel nested paths, and common MV3 permissions. Semantic recognition was expanded so priority top-level keys render as known fields, and optional permission/host-permission array items classify like install-time permission arrays. Remaining work: deeper nested paths, long-tail/browser-specific permissions, and value-level enum explanations once semantic value support is ready.
